...
%s链上地址%s在北京时间%s向%s交易所地址%s转入了%s万个%s,目前市值为%s万美元,警惕砸盘风险。'%(alert,blockchain,from_address_now,localtime_now,to_address_owner_now,to_address_now,amount_now,currecy_now,amount_usd_now)
                                        ...
                                        df = pd.read_csv('/root/whale-alert/res_alert.csv')
                                        sub_df = pd.DataFrame({'date':now_time,'crypto':currecy_now,'exchange':to_address_owner_now,'number':amount_now,'value':amount_usd_now,'hash':hash_v},index=[0])
...
%s链上地址%s在北京时间%s向%s交易所地址%s转入了%s万个%s,目前市值为%s万美元。'%(alert,blockchain,from_address_now,localtime_now,to_address_owner_now,to_address_now,amount_now,currecy_now,amount_usd_now)
                                        ...
                                        now_time = str(datetime.utcnow())[0:19]
                                        df = pd.read_csv('/root/whale-alert/res_alert.csv')
                                        sub_df = pd.DataFrame({'date':now_time,'crypto':'USDT','exchange':to_address_owner_now,'number':amount_now,'value':amount_usd_now,'hash':hash_v},index=[0])
                                        df = pd.concat([df,sub_df])
                                        df.to_csv('/root/whale-alert/res_alert.csv',index=False)
                            else:
                                continue
